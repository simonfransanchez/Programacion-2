...
// EJERCICIO 2
void ej2(arbolN A, pos p, int n, int k, int *max, int *min){ // llamado ej2(A,raiz(A),1,k,*max,*min) min=999 max=0 (main)
    pos c;
    int gr = 0;
    if (!nulo(p) && n <= k){
        c = hijomasizq(p);
...
// EJERCICIO 3
void determina(int mat[][MAX], int n, int i, int j, int *g1, int *g2){ //llamado deetermina(mat,n-1,n-2,n-1)
    if (i>=0 && (*g1 + *g2 != 0))
        if (i == j)
            determina(mat,n,i-1,n,g1,g2);
        else{
            if (mat[i][j] > mat[j][i])
                *g1 = 0;
            else
